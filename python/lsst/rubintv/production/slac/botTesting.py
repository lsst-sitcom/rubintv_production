...
PER_IMAGE_HEADERS = {'OBSID': 'Observation Id',
                     ...
                     'CATKW101': 'Carriage axis B',
                     'CATKW103': 'Carriage axis U',
                     'CATKW10D': 'Incidence angle FP',
                     'CATKW10E': 'FP x-coord',
                     'CATKW10F': 'FP y-coord',
                     'CATKW110': 'Incidence angle FS1',
                     'CATKW128': 'Convergence test',
                     'CATKW129': 'CCOB narrow flash time (commanded)',
                     'CATKW12A': 'Exptime (requested)',
                     'CATKW12B': 'Filter model',
                     'CATKW12C': 'CCOB catalog exposure id',
                     'CATKW12D': 'Hyperchromator wavelength',
                     'CATKW130': 'Sensor address',
                     'CATKW131': 'Target photon level',
                     }
...
class Plotter:
    ...
    def __init__(self, butler, locationConfig, instrument, doRaise=False):
        ...
        self.STALE_AGE_SECONDS = 30  # in seconds
    ...
    def writeCcobAnalysisShard(self, expRecord, image, spotInfo, binning, camera):
        ...
        detName = None
        detector = getDetectorForBinnedImageLocation(image, spotInfo.centerOfMassMosaic, binning, camera)
    ...
    def callback(self, expRecord, doPlotMosaic=False, doPlotNoises=False, doCcobAnalysis=False, timeout=5):
        ...
        if doPlotNoises:
            ...
            if noiseMapFile:
                ...
                self.uploader.uploadPerSeqNumPlot(channel, dayObs, seqNum, noiseMapFile, isLiveFile=True)
        ...
        if doPlotMosaic:
            ...
            if focalPlaneFile:
                ...
                self.uploader.uploadPerSeqNumPlot(channel, dayObs, seqNum, focalPlaneFile, isLiveFile=True)
